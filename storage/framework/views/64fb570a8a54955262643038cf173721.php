...
<form class="mt-10 mx-auto w-full max-w-sm" action="<?php echo e(route('movie.reserch')); ?>" method="GET">
  <div class="flex items-center border-b border-indigo-500 py-2">
    <input class="text-center appearance-none font-semibold  bg-transparent border-none w-full text-white mr-3 py-1 px-2 leading-tight focus:outline-none" type="text" name="query" placeholder="映画名を入力してください" aria-label="Full name" value="<?php echo e(old('query', $query)); ?>" >
    ...
    <?php if(isset($allMovie)): ?>
        <?php if(count($allMovie) > 0): ?>
            <?php $__currentLoopData = $allMovie; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $movie): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
              <?php if(['movie_path'] !== ''): ?>
...
<?php echo $__env->make('layout.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/resources/views/movie/reserch.blade.php ENDPATH**/ ?>